...
"""
Функция, создающая матрицу размером с размером поля и номерами -1 в клетках, в которые робот может попасть
и номером компоненты связности в клетках, в которые робот попасть не может
\n
\n///INPUT///
\nr - объект робота
\n
\n///OUTPUT///
\nМатрица, размером поля, на котором находится робот с -1 в точках, 
в которые робот может попасть и номером компоненты связности в точках, в которые робот не может попасть (считая от 1) 
"""
function create_map!(robot::Robot)::Matrix{Int}
    ...
    """
    Обход в глубину, расставляющий номера компонент связности в матрице.
    """
    function dfs(x, y)
